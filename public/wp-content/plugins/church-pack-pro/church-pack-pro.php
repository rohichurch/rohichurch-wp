...
define( 'WPFC_STORE_URL', 'http://www.wpforchurch.com' ); // you should use your own CONSTANT name, and be sure to replace it throughout this file

// the name of your product. This should match the download name in EDD exactly
define( 'WPFC_ITEM_NAME', 'Church Pack Pro' ); // you should use your own CONSTANT name, and be sure to replace it throughout this file
...
$license_key = trim( get_option( 'churchpack_license_key' ) );

// setup the updater
$edd_updater = new EDD_SL_Plugin_Updater( WPFC_STORE_URL, __FILE__, array( 
		'version' 	=> '1.2.4', 				// current version number
		'license' 	=> $license_key, 		// license key (used get_option above to retrieve from DB)
		'item_name' => WPFC_ITEM_NAME, 	// name of this plugin
		'author' 	=> 'WP for Church'  // author of this plugin
	)
);

// Include Events
$events = isset($coreoptions['events']);
if ( $events == '1' ) { 
require_once plugin_dir_path( __FILE__ ) . '/events.php';
add_action( 'wp_head', 'wpfc_churchpack_scripts' );
}
...
function wpfc_churchpack_scripts() {
	...
	if($post) {
		if ( false !== strpos($post->post_content, '[events') ) {	
			...
			$event_ajax = ('http://www.rohichurch.org/wp-admin/admin-ajax.php' );
			?> 
