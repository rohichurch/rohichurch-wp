...
<div class="wp-badge"><?php _e( 'Church Pack<br/> v. 1.2.4' ); ?></div>
...
<div class="changelog">
	...
	<!-- Events -->
			...
			<label><input id="events" name="cpo_core_options[events]" type="checkbox" value="1" <?php if (isset($coreoptions['events'])) { checked('1', $coreoptions['events']); } ?> /> <?php _e('Enable Events', 'churchpack'); ?></label><br />
...
function cpo_render_churchinfo() {
	...
	<div class="wrap">
		...
		<p><?php _e('If you need help, please visit <a href="http://www.wpforchurch.com/" target="_blank">WP for Church</a>', 'churchpack'); ?></p>
...
function cpo_render_support_page() {
	...
	<form method="post" action="options.php">
			...
			<?php 
				$license 	= get_option( 'churchpack_license_key' );
				$status 	= get_option( 'churchpack_license_status' );
				settings_fields('churchpack_license'); 
			?>
			...
			<table class="form-table">
				<tbody>
					<tr valign="top">	
						...
						<td>
							<input id="churchpack_license_key" name="churchpack_license_key" type="text" class="regular-text" value="<?php esc_attr_e( $license ); ?>" />
							<label class="description" for="churchpack_license_key"><?php _e('Enter your license key to get automatic updates'); ?></label>
						</td>
					</tr>
						...
						<tr valign="top">	
							...
							<td>
								<?php if( $status !== false && $status == 'valid' ) { ?>
									...
									<?php wp_nonce_field( 'churchpack_nonce', 'churchpack_nonce' ); ?>
									<input type="submit" class="button-secondary" name="churchpack_license_deactivate" value="<?php _e('Deactivate License'); ?>"/>
								<?php } else {
									wp_nonce_field( 'churchpack_nonce', 'churchpack_nonce' ); ?>
									<input type="submit" class="button-secondary" name="churchpack_license_activate" value="<?php _e('Activate License'); ?>"/>
								<?php } ?>
							</td>
						</tr>
					<?php } ?>
...
function churchpack_license( $new ) {
	$old = get_option( 'churchpack_license_key' );
	if( $old && $old != $new ) {
		delete_option( 'churchpack_license_status' ); // new license has been entered, so must reactivate
	}
	return $new;
}
...
function churchpack_activate_license() {

	// listen for our activate button to be clicked
	if( isset( $_POST['churchpack_license_activate'] ) ) {

		// run a quick security check 
	 	if( ! check_admin_referer( 'churchpack_nonce', 'churchpack_nonce' ) ) 	
			return; // get out if we didn't click the Activate button

		// retrieve the license from the database
		$license = trim( get_option( 'churchpack_license_key' ) );
		...
		$api_params = array( 
			...
			'license' 	=> $license, 
			'item_name' => urlencode( WPFC_ITEM_NAME ) // the name of our product in EDD
		);
		...
		update_option( 'churchpack_license_status', $license_data->license );

	}
}
add_action('admin_init', 'churchpack_activate_license');
...
function churchpack_deactivate_license() {

	// listen for our activate button to be clicked
	if( isset( $_POST['churchpack_license_deactivate'] ) ) {
		...
		if( $license_data->license == 'deactivated' )
			delete_option( 'churchpack_license_status' );

	}
}
add_action('admin_init', 'churchpack_deactivate_license');
