...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<?php if(has_post_thumbnail()) : ?>
			<?php if ( !is_single() ) : // No Link on Single View ?>
				<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_post_thumbnail('content-width', array('class' => 'post-thumb', 'alt' => ''.get_the_title().'', 'title' => ''.get_the_title().'')); ?></a>
			<?php else : ?>
				<?php the_post_thumbnail('content-width', array('class' => 'post-thumb', 'alt' => ''.get_the_title().'', 'title' => ''.get_the_title().'')); ?>
			<?php endif; ?>		    	
	<?php endif; ?>	
	...
	<footer class="entry-meta">
		<?php if ( 'post' == get_post_type() ) : // Hide category and tag text for pages on Search ?>
			...
			<span class="cat-links">
				<?php printf( __( '<i class="icon-folder-open"></i>  %1$s', 'edwards' ), $categories_list ); ?>
			</span>
			...
			<span class="tags-links">
				<?php printf( __( '<i class="icon-tags"></i> %1$s', 'edwards' ), $tags_list ); ?>
			</span>
